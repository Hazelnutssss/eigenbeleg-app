...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sourceFolder = folderSearch.files?.find(f => f.name.includes('Screenshots') && f.name.includes('Ankauf') && !f.name.includes('abgearbeitet'));
    if (!sourceFolder) throw new Error(`Ordner "${SOURCE_FOLDER_NAME}" nicht gefunden`);
    ...
    // 3. Alle Dateien im SKU-Ordner abrufen (includeItemsFromAllDrives für bessere Sichtbarkeit)
    const filesSearch = await gFetch(
      `https://www.googleapis.com/drive/v3/files?q='${skuFolder.id}' in parents and trashed=false&fields=files(id,name,mimeType)&orderBy=name&includeItemsFromAllDrives=true&supportsAllDrives=true`,
      token
    );
    const files = filesSearch.files || [];
    if (!files.length) throw new Error(`Keine Dateien in Ordner "${skuFolder.name}" (Ordner-ID: ${skuFolder.id})`);

    // 4. Nur Metadaten zurückgeben - Bilder werden direkt von eb-extract geladen
