...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Quellordner in Drive finden (name contains für Robustheit mit Sonderzeichen)
    const folderSearch = await gFetch(
      `https://www.googleapis.com/drive/v3/files?q=name contains 'Ankauf' and mimeType='application/vnd.google-apps.folder' and trashed=false&fields=files(id,name)`,
      token
    );
    // Exakten Ordner aus den Ergebnissen filtern
    const sourceFolder = folderSearch.files?.find(f =>
      f.name === SOURCE_FOLDER || f.name.includes('Screenshots + weiteres') && f.name.includes('Ankauf') && !f.name.includes('abgearbeitet')
    );
    if (!sourceFolder) throw new Error(`Ordner "${SOURCE_FOLDER}" nicht gefunden. Gefundene Ordner: ${folderSearch.files?.map(f=>f.name).join(', ')}`);
    ...
    const skuPattern = /([A-Z]+-[A-Z0-9]+-\d{5})/i;
    ...
    for (const sheet of sheets) {
      ...
      try {
        const resp = await gFetch(
          `${base}/values/${encodeURIComponent("'" + title + "'!A1:BZ500")}`,
          token
        );
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          sheetsIndex[skuCell.toUpperCase()] = {
            ...
            seriennummer: (rows[i][52] || '').toString().trim(), // Spalte BA = Index 52
          };
        }
      } catch(e) { continue; }
    }
    ...
    try {
      ...
      if (belnrRows.length > 0) {
        ...
        // Kompletten Text des Vorgängers übernehmen, nur die letzte Zahlengruppe +1.
        // So bleibt jeder Präfix erhalten (z.B. "EB-2025-047" -> "EB-2025-048",
        // "EB-2026-003" -> "EB-2026-004"), unabhängig vom Jahr oder Format.
        const match = lastVal.match(/^(.*?)(\d+)(\D*)$/);
        if (match) {
          const prefix = match[1];       // alles vor der letzten Zahlengruppe, z.B. "EB-2025-"
          const numStr = match[2];       // die letzte Zahlengruppe, z.B. "047"
          const suffix = match[3] || ''; // evtl. Text nach der Zahl (normalerweise leer)
          const incremented = String(parseInt(numStr, 10) + 1).padStart(numStr.length, '0');
          nextBelnr = `${prefix}${incremented}${suffix}`;
        } else if (lastVal) {
          // Fallback: Vorgänger enthält keine Zahl – unverändert übernehmen
          nextBelnr = lastVal;
        }
      }
    } catch(e) {}
