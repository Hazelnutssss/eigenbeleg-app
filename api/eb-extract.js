  ...
  try {
    ...
    for (const fileId of fileIds) {
      try {
        ...
        if (!isImage && !isPdf) {
          metaCache.push({ fileId, mimeType: metaData.mimeType, name: metaData.name, isPdf: false, skipped: true });
          continue;
        }
        metaCache.push({ fileId, mimeType: metaData.mimeType, name: metaData.name, isPdf });
      } catch(e) {
        continue;
      }
    }

    const skippedFiles = metaCache.filter(f => f.skipped);
    const usableMeta = metaCache.filter(f => !f.skipped);

    // PDFs zuerst einsortieren, dann Bilder – so werden PDFs bevorzugt in die ersten 4 geladenen Dateien aufgenommen
    usableMeta.sort((a, b) => (b.isPdf ? 1 : 0) - (a.isPdf ? 1 : 0));

    for (const { fileId, mimeType, name, isPdf } of usableMeta.slice(0, 4)) {
      try {
        ...
        loadedFiles.push({ mimeType, name, data: b64, kind: isPdf ? 'document' : 'image' });
      } catch(e) {
        continue;
      }
    }
    ...
    if (!loadedFiles.length) {
      return res.status(200).json({
        ...
        _debug: {
          uebersprungeneDateien: skippedFiles.map(f => ({ name: f.name, mimeType: f.mimeType })),
          gefundeneDateiTypen: usableMeta.map(f => ({ name: f.name, mimeType: f.mimeType }))
        }
      });
    }
...
Falls ein Feld nicht zweifelsfrei erkennbar ist: leeren String zurückgeben, nicht raten.`;
    ...
    return res.status(200).json({
      ...
      ...extracted,
      _debug: {
        geladeneDateien: loadedFiles.map(f => ({ name: f.name, mimeType: f.mimeType, kind: f.kind })),
        uebersprungeneDateien: skippedFiles.map(f => ({ name: f.name, mimeType: f.mimeType })),
        hasPdf,
        rohantwortClaude: text
      }
    });
