...
function buildPDF({ belnr, sku, artikel, datum, betrag, plattform, zahlungsart,
  ...
  // Teilrückzahlung: tatsächlichen Einkaufspreis berechnen (nur wenn Betrag erfasst wurde)
  const hatRueckzahlung = !!(rueckzahlungBetrag && parseEuro(rueckzahlungBetrag) !== null);
  let rueckzahlungBlockDeckblatt = '';
  let rueckzahlungBlockBeleg = '';
  let begruendungText = `Ankauf von Privatperson (nicht gewerblich);\nVerkäufer stellt üblicherweise keine Rechnung/Quittung aus.`;
...
Tatsächlicher Einkaufspreis: ${tatsaechlich !== null ? formatEuro(tatsaechlich) : '–'}
`;

    rueckzahlungBlockBeleg = `
TEILRÜCKZAHLUNG
Ursprünglicher Kaufpreis: ${betrag} (gezahlt am ${datum})
Teilrückzahlung: -${rueckzahlungBetrag} (erhalten am ${rueckzahlungDatum || '–'})
Tatsächlicher Einkaufspreis (maßgeblich für §25a-Marge): ${tatsaechlich !== null ? formatEuro(tatsaechlich) : '–'}
`;

    begruendungText = `Ankauf von Privatperson (nicht gewerblich);\nVerkäufer stellt üblicherweise keine Rechnung/Quittung aus.\n\n${rueckzahlungGrund || `Ware entsprach nicht der Beschreibung; Verkäufer leistete am ${rueckzahlungDatum || '–'} eine Teilerstattung von ${rueckzahlungBetrag} zurück.`}`;
  }
