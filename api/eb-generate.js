...
function drawTextPage(doc, font, fontBold, title, lines, signatureImage = null) {
  ...
  for (const item of lines) {
    ...
    if (item.signature) {
      // Unterschrift-Grafik direkt über der folgenden Linie platzieren
      if (signatureImage) {
        const maxW = 180, maxH = 55;
        ...
        // knapp über die Unterschriftslinie setzen (Linie kommt als nächste Zeile bei y - lineHeight)
      ...
      // Kein y-Vorschub: die Linie darunter wird von der nächsten Zeile gezeichnet
      continue;
    }
    if (item.heading) {
      y -= 4;
