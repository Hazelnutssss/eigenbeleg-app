...
export default async function handler(req, res) {
  ...
  const { password, sheetId, sheetName, sheetRow, belnr, sku, artikel, datum,
          ...
          folderDriveId, rueckzahlungBetrag, rueckzahlungDatum, rueckzahlungGrund,
          seriennummer } = req.body || {};
  ...
  try {
    ...
    const pdfBytes = await buildPDF({
      ...
      rueckzahlungBetrag, rueckzahlungDatum, rueckzahlungGrund, seriennummer
    });
    ...
    // 6. Quellordner nach "abgearbeitet" verschieben (Fallback: in den Papierkorb)
    // Verschieben wird zuerst versucht. Scheitert es (z.B. weil der SKU-Ordner dem
    // User-Token unter drive.file nicht zugänglich ist), wird der Ordner stattdessen
    // in den Papierkorb gelegt – 30 Tage wiederherstellbar. Da das Konto reiner
    // Automatisierungsspeicher ist, ist das unkritisch; die Daten sind im PDF konserviert.
    if (folderDriveId) {
      let verschoben = false;
      let moveFehler = '';

      // 6a. Verschieben versuchen (mit User-Token, da SKU-Ordner ggf. per Picker zugänglich)
      try {
        const DONE_FOLDER = 'Screenshots + weiteres - Ankauf - abgearbeitet';
        const doneFolderSearch = await gFetch(
          `https://www.googleapis.com/drive/v3/files?q=name='${DONE_FOLDER}' and mimeType='application/vnd.google-apps.folder' and trashed=false&fields=files(id,name)&supportsAllDrives=true&includeItemsFromAllDrives=true`,
          userToken
        );
        let doneFolderId;
        if (doneFolderSearch.files?.length) {
          ...
          if (!createResp.ok) throw new Error(createData.error?.message || 'Konnte "abgearbeitet"-Ordner nicht erstellen');
          doneFolderId = createData.id;
        }

        const folderMeta = await gFetch(
          `https://www.googleapis.com/drive/v3/files/${folderDriveId}?fields=parents&supportsAllDrives=true`,
          userToken
        );
        const oldParents = (folderMeta.parents || []).join(',');

        const moveResp = await fetch(
          `https://www.googleapis.com/drive/v3/files/${folderDriveId}?addParents=${doneFolderId}${oldParents ? '&removeParents=' + oldParents : ''}&fields=id&supportsAllDrives=true`,
        ...
        if (moveResp.ok) {
          ...
          moveFehler = moveData.error?.message || 'unbekannt';
        }
      } catch(moveErr) {
        moveFehler = moveErr.message;
      }

      // 6b. Fallback: in den Papierkorb legen, falls Verschieben scheiterte
      if (!verschoben) {
        try {
          const trashResp = await fetch(
            `https://www.googleapis.com/drive/v3/files/${folderDriveId}?supportsAllDrives=true`,
            {
              method: 'PATCH',
          ...
          if (!trashResp.ok) {
            // Zweiter Versuch mit User-Token, falls Service-Account keine Rechte hat
            await fetch(
              `https://www.googleapis.com/drive/v3/files/${folderDriveId}?supportsAllDrives=true`,
              {
                method: 'PATCH',
          ...
          console.error('Papierkorb-Fallback fehlgeschlagen:', trashErr.message);
          return res.status(200).json({
            success: true, filename, belnr, driveFileId: uploadData.id,
            warnung: 'PDF erstellt und Belegnummer eingetragen, aber Ordner konnte weder verschoben noch in den Papierkorb gelegt werden.'
          });
        }
      }
    }
...
async function buildPDF({ belnr, sku, artikel, datum, betrag, plattform, zahlungsart,
  ...
  // Unterschrift laden (feste Datei-ID aus Umgebungsvariable), falls vorhanden
  let signatureImage = null;
  const signatureFileId = process.env.SIGNATURE_FILE_ID;
  if (signatureFileId) {
    try {
      const sigBytes = await downloadDriveFile(signatureFileId, token);
      if (sigBytes) {
        try {
          signatureImage = await doc.embedPng(sigBytes);
        } catch(pngErr) {
          // Falls die Datei trotz .png-Endung ein JPG ist
          signatureImage = await doc.embedJpg(sigBytes);
        }
      }
    } catch(sigErr) {
      console.error('Unterschrift konnte nicht geladen/eingebettet werden:', sigErr.message);
      signatureImage = null;
    }
  }

  // Seite 1: Deckblatt
  drawTextPage(doc, font, fontBold, 'EIGENBELEG – ÜBERSICHT', [
  ...
  drawTextPage(doc, font, fontBold, 'EIGENBELEG', [
    ...
    { label: 'Artikelbeschreibung', value: artikel },
    { label: 'SKU', value: sku },
    { label: 'Seriennummer', value: seriennummer || 'keine Seriennummer' },
    { label: 'Plattform', value: plattform },
    { label: 'Zahlungsart', value: zahlungsart },
...
function drawTextPage(doc, font, fontBold, title, lines, signatureImage = null) {
  const page = doc.addPage([595, 842]);
  const marginX = 55;
  let y = 790;
  const lineHeight = 15;
  ...
  for (const item of lines) {
    ...
    if (item.signature) {
      // Unterschrift-Grafik über der folgenden Linie platzieren
      if (signatureImage) {
        const maxW = 200, maxH = 75;
        ...
        // Unterkante der Unterschrift knapp über die Linie (Linie liegt bei y - lineHeight)
      ...
      // Etwas Extra-Abstand, damit die Unterschrift Luft nach oben hat
      y -= lineHeight;
      continue;
    }
    if (item.heading) {
